...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            ...
            if hasattr(m, 'backbone'): #---------------------ADA TAMBAHAN-----------------------
                x = m(x)
                ...
                x = m(x)  # run
                y.append(x if m.i in self.save else None)  # save output #---------------------ADA TAMBAHAN-----------------------
            if visualize:
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    is_backbone = False #----------------------------------------ADA TAMBAHAN-----------------------------------------
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c1 = ch[f]
            args = [*args[1:]]
        elif isinstance(m, str): #----------------------------------------ADA TAMBAHAN-----------------------------------------
            t = m
            if len(args) == 2:        
        ...
        if isinstance(c2, list):
            ...
            ch.append(c2)
    return torch.nn.Sequential(*layers), sorted(save) #----------------------------------------ADA TAMBAHAN-----------------------------------------
